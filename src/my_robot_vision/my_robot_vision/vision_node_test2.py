    ...
    def run(self):
        for task in self.task_list:
            ...
            while os.path.exists(RESULT_FILE):
                print("wait for old task 等待旧任务完成信号清除...")
                time.sleep(1.0)
            ...
            # --- 阶段 1: 目标检测 (GroundingDINO) ---
            print(" step1: detecting 阶段 1: 正在检测目标...")
            obs_start = time.time()
            ...
            cv2.imshow("Vision", viz_det)
            print(" successfullt recognized 检测成功，展示结果中...")
            cv2.waitKey(2000) # 停顿 2 秒

            # --- 阶段 2: 分割 (SAM) ---
            print("step2:generate mask 阶段 2: 正在生成掩码...")
            if SAM_AVAILABLE:
                ...
                print("segmentation successfully 分割成功，展示结果中...")
                cv2.waitKey(2000) # 停顿 2 秒
            else:
            ...
            print(" step3: estimating 6d pose 阶段 3: 正在精炼 6D 姿态...")
            ...
            if pose_samples:
                ...
                print(f"end of vision_pipeline,send task 视觉流程结束，发送任务: {name}")
                cv2.waitKey(2000) # 停顿 2 秒
